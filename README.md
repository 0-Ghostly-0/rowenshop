...
**This one publishes itself — you just approve or reject with one click, nothing to edit or push.** Submitting uploads the files straight to Blob storage (Vercel's file storage — the same place the VST downloads live), then emails you every field they filled in plus two links: **Approve & publish** and **Reject**. Click Approve and the beat shows up on rowen.work within a few seconds; click Reject (or ignore the email) and nothing happens — the submission and its files are simply discarded. There's no dashboard, login, or separate step: the email link IS the whole review process.

This is separate from beats you add yourself in `config.js` (the "Adding a new beat" section above) — the site shows both lists combined. Nothing stops you from using the same submission form for your own beats too, if you'd rather go through one consistent flow (just leave the "producer" fields blank/blank — a beat with no producer credited shows exactly like ones you add directly).

**One-time setup**, in your Vercel project → Settings → Environment Variables (in addition to `BLOB_READ_WRITE_TOKEN`, `RESEND_API_KEY`, and `EMAIL_FROM`, which you likely already have from the VST store — see "Selling a finished VST" below):
- `BEAT_APPROVAL_SECRET` — any long random string you make up (same idea as `LICENSE_SECRET` — e.g. `openssl rand -hex 32`). This is what signs the Approve/Reject links so nobody else can guess or forge one.
- `BEAT_REVIEW_EMAIL` — the inbox address that should receive these review emails (probably your own).

Until both of those are set, submitting the form will fail with a clear "not fully set up yet" message rather than silently losing the submission or pretending it worked.

**Worth knowing about what "automatic" does and doesn't cover:** approving a submission publishes its info and audio preview — it does NOT automatically set up a way to actually buy that beat. A freshly-approved beat shows an "Ask on Discord" button until you add a `stripeLink` for it, same as any beat added by hand (see "Hosting other producers' beats" below if it's not your own beat, since that's also where the revenue-split conversation happens). Also, once a beat is auto-published this way, editing or taking it back down isn't self-service yet from anywhere in this repo — ask me in a session to fix a specific entry if that comes up, or mention it and I can add a proper way to manage it later.
